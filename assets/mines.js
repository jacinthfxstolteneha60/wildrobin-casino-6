...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-c9bb4456";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-c9bb4456">'+'<span class="tile-face-c9bb4456 tile-front-c9bb4456"></span>'+'<span class="tile-face-c9bb4456 tile-back-c9bb4456"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-c9bb4456")){return;}
var back=tile.querySelector(".tile-back-c9bb4456");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-c9bb4456");if(isMine){tile.classList.add("boom-c9bb4456");}
if(dim){tile.classList.add("dimmed-c9bb4456");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-c9bb4456")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-c9bb4456");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-c9bb4456")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-c9bb4456");void panelEl.offsetWidth;panelEl.classList.add("jolt-c9bb4456");}
endRound();}
